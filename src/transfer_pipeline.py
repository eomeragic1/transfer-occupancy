...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_transfer', type=bool, default=False)
...
#         os.system(cl_builder_source)
    ...
    #                        --lr {lr} --batch_size {batch_size} --hidden_size {hidden_size} \
    #                        --epoch {transfer_epoch if train_days!=-1 else 0} --train_days {train_days} --val_days 5 --visualize False --transfer False \
    #              --transfer_path ../model_checkpoints/{dataset_name}_{model}_2023-01-31_seed_888888_traindays_None_9189.pt \
    #               --buildings "Household 01" "Household 03" "Household 06" --seed {seed} --num_frozen_layers 0'
    #
    #         os.system(cl_builder_transfer)

    if eval_transfer:
        for num_frozen_layers, transfer_lr in zip([1], transfer_lr_list):
            # train_days_list = [2, 4, 6, 8, 10]
            # for train_days in train_days_list:
            for seed in seeds:
                print(f'python -u ./main.py --model {model} --dataset {dataset_name} \
                                                --lr {transfer_lr} --batch_size {batch_size} --hidden_size {hidden_size} \
                                                --epoch {transfer_epoch if train_days!=-1 else 0} --train_days {train_days} --val_days 5 --visualize False --transfer True \
                                      --transfer_path ../model_checkpoints/HPDMobile_LSTM_2023-01-31_seed_888888_traindays_None_9189.pt \
                                       --buildings "Household 01" "Household 03" "Household 06" --seed {seed} --num_frozen_layers {num_frozen_layers}')
